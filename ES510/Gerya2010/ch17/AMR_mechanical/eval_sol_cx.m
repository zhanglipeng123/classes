function sol = eval_sol_cx( x, z )

PI = 3.14159265358979323846264338328;

%  Status:
% 	u,v - flow field direction is correct
%   p - magnitude and locations look correct
%
%  eta = eta_A if x <= x_c
%  eta = eta_B if x > x_c
%  rho = cos( nx.pi.x ).sin( nz.pi.z )
%  nx : integer
%  nz : double

% double pos[],
% double _eta_A, double _eta_B, 	    % Input parameters: density, viscosity A, viscosity B %
% double _x_c, int _nx, double _nz, 	% Input parameters: viscosity jump location (in x), wavenumber in x,z %
% double vel[], double* pressure, double total_stress[], double strain_rate[] )


% del_rho = sin(n*Pi*z)*cos(nx*Pi*x)  n=nx gives only non-zero terms%
n  = 1; % only one n in Fourier series because  del_rho has cos term %
nx = 1; % can set these two to whatever we like %

ZA = 1;   % left column viscosity %
ZB = 1e6; % right column viscosity %
xc = 0.5;
...
if(x>=xc) % Here I had to modify the condition (x>xc) in order to produce the same results as the C code.
    C1=C1B; C2=C2B; C3=C3B; C4=C4B; Z=ZB;
else
    C1=C1A; C2=C2A; C3=C3A; C4=C4A; Z=ZA;
end
%**************************************************************************************%
%**************************************************************************************%
t1 = n * n;
...
u5 = (-2*Z*n*PI*u2-u3*2*n*PI)*cos(n*PI*z); % pressure %

u6 = (u3*2*n*PI + 4*Z*n*PI*u2)*cos(n*PI*z); % zz stress %
sum5 = sum5 + u5;
sum6 = sum6 + u6;

u1 = u1 * cos(n*PI*z); % x velocity
sum1 = sum1 + u1;
u2 = u2 * sin(n*PI*z); % z velocity
sum2 = sum2 + u2;
u3 = u3 * 2*n*PI*cos(n*PI*z); % xx stress
sum3 = sum3 + u3;
u4 = u4 * 2*n*PI*sin(n*PI*z); % zx stress
sum4 = sum4 + u4;
