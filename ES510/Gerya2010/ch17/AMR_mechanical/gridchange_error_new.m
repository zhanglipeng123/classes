...
% Mark cells for refinement based on errors
for ci=1:1:celnum
    if(celdot(ci,1)==0)  
        % No refinement mark
        % Mark high-error cell for refinement
        if(SE(celvar(ci,1))>=vxerrmax || SE(celvar(ci,4))>=vxerrmax || ...
           SE(celvar(ci,2))>=vyerrmax || SE(celvar(ci,3))>=vyerrmax || ...
           SE(celvar(ci,5))>=prerrmax)
