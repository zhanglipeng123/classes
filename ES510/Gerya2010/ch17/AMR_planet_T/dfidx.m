% Function dfidx()
% This function compose dFI/dx for specific cell boundary
% and add it to the global matrix
% Function return modified matrix
function[L]=dfidx(kp,ciL1,ciL2,ciR1,ciR2,nodx,nody,celnod,celvar,celres,pkf,L,binner,bouter,xsize,ysize)
...
if(celres(ciL1)>celres(ciR1))
    ...
    if(cdistR1<binner)
        if(cdistL1<binner && cdistL2<binner)
            ...
            % Add FIR1, FIL1, FIL2
            L(kp,celvar(ciR1,6))=L(kp,celvar(ciR1,6))+pkf/dx;
            L(kp,celvar(ciL1,6))=L(kp,celvar(ciL1,6))-pkf/dx/2;
            L(kp,celvar(ciL2,6))=L(kp,celvar(ciL2,6))-pkf/dx/2;
        else
            % Left boundary condition
            % Define horizontal distance  
            % to the Left gravity potential boundary
