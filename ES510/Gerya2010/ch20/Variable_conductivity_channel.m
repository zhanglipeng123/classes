...
MCP(1)=1000;
% Rock thermal conductivity, k=k0/(1+b*(T-T0)/T0), W/m/K
MKT(1,1)=8; % k0
MKT(1,2)=1; % b
MKT(1,3)=ttop; % T0
% Maximal timestep, s
timemax=1e+5*(365.25*24*3600);
    ...
    for xm = 1:1:mxnum
        for ym = 1:1:mynum 
            ...
            if (MX(ym,xm)>=0 && MX(ym,xm)<=xsize && MY(ym,xm)>=0 && MY(ym,xm)<=ysize) 
                ...
                % Rock thermal conductivity, k=k0/(1+b*(T-T0)/T0), W/m/K
                MKTCUR=MKT(MI(ym,xm),1)/(1+MKT(MI(ym,xm),2)*(MTK(ym,xm)-MKT(MI(ym,xm),3))/MKT(MI(ym,xm),3));
                ...
                kt1(yn,xn)=kt1(yn,xn)+(1.0-dx).*(1.0-dy).*MKTCUR;
                ...
                kt1(yn+1,xn)=kt1(yn+1,xn)+(1.0-dx).*dy.*MKTCUR;
                ...
                kt1(yn,xn+1)=kt1(yn,xn+1)+dx.*(1.0-dy).*MKTCUR;
                ...
                kt1(yn+1,xn+1)=kt1(yn+1,xn+1)+dx.*dy.*MKTCUR;
