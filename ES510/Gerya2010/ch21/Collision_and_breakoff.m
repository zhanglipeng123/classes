...
% Setup corresponds to continental collision 
% 
% Staggered Grid 
% 
%     vx       vx       vx    
...
velcol=3/(100*365.25*24*3600); % 2 cm/yr
...
% Time to stop convergence (transition to breakoff)
timestop=12.7e+6*365.25*24*3600; % 12.7 Myr

% Viscoelastic timestep, s
timemax=1e+4*365.25*24*3600; % 5000 year
...
MPL(2,1)=1e+6;              % C0, Pa
MPL(2,2)=1e+6;              % C1, Pa
MPL(2,3)=0.03;                 % sin(FI0)
MPL(2,4)=0.03;                 % sin(FI1)
MPL(2,5)=0;                 % GAM0
MPL(2,6)=1;                 % GAM1
MCP(2)=1000;                % Cp, J/kg
...
MPL(3,1)=1e+6;              % C0, Pa
MPL(3,2)=1e+6;              % C1, Pa
MPL(3,3)=0.03;                 % sin(FI0)
MPL(3,4)=0.03;                 % sin(FI1)
MPL(3,5)=0;                 % GAM0
MPL(3,6)=1;                 % GAM1
MCP(3)=1000;                % Cp, J/kg
...
MPL(7,1)=1e+6;              % C0, Pa
MPL(7,2)=1e+6;              % C1, Pa
MPL(7,3)=0.03;                 % sin(FI0)
MPL(7,4)=0.03;                 % sin(FI1)
MPL(7,5)=0;                 % GAM0
MPL(7,6)=1;                 % GAM1
MCP(7)=1000;                % Cp, J/kg
...
for j=1:1:xnum+1
    ...
    bbottom(j,3)=velcol/xsize*ysize;
    bbottom(j,4)=0;
end
...
gridx(26)=200000; % initial position of the area
for i=27:1:176
    gridx(i)=gridx(i-1)+b;
end
% Define factor of grid spacing increase to the right
% of high resolution area
D=xsize-gridx(176); % distance to be covered by non-uniform grid
N=25; % number of grid steps to be used in the grid
...
for i=177:1:xnum
    gridx(i)=gridx(i-1)+b*F.^(i-176);
end
gridx(xnum)=xsize;
% Define factor of grid spacing increase to the left
% of high resolution area
D=gridx(26); % distance to be covered by non-uniform grid
N=25; % number of grid steps to be used in the grid
% Iterative search of F
F=1.1;
for i=1:1:100
...
% of high resolution area
D=ysize-gridy(31); % distance to be covered by non-uniform grid
...
mxnum=1000; %number of markers in vertical direction
mxstep=xsize/mxnum; %number of markers in horizontal direction
mynum=300;  %step between markers in horizontal direction   
...
for xm = 1:1:mxnum
    for ym = 1:1:mynum
        ...
        % 3 = Gabbroic crust
        if(MY(mm1)>13000 && MY(mm1)<=18000)
            MI(mm1)=4;
        end
        % 5 = Lithosphere 
        if(MY(mm1)>18000 && MY(mm1)<80000) 
            MI(mm1)=5;
        end
        % Left continent
        % 8 = upper continental crust 
        if(MX(mm1)<350000 && MY(mm1)>7000 && MY(mm1)<27000) 
            MI(mm1)=8;
        end
        if(MX(mm1)>=350000 && MX(mm1)<400000 && MY(mm1)>7000+(MX(mm1)-350000)/50000*3000 && MY(mm1)<32000-(MX(mm1)-350000)/50000*14000) 
            MI(mm1)=8;
        end
        % 9 = lower continental crust 
        if(MX(mm1)<350000 && MY(mm1)>27000 && MY(mm1)<42000) 
            MI(mm1)=9;
        end
        if(MX(mm1)>=350000 && MX(mm1)<400000 && MY(mm1)>27000-(MX(mm1)-350000)/50000*14000 && MY(mm1)<42000-(MX(mm1)-350000)/50000*24000) 
            MI(mm1)=9;
        end
        % Right continent
        % 8 = upper continental crust 
        ...
        if(MY(mm1)>18000 && MY(mm1)<80000 && MX(mm1)>390000-(MY(mm1)-18000)/24000*50000 && MX(mm1)<410000-(MY(mm1)-18000)/24000*50000) 
            MI(mm1)=7;
        end
        % 3 = Weak zone in the oceanic crust
        if(MY(mm1)>10000 && MY(mm1)<=18000 && MX(mm1)>390000-(MY(mm1)-18000)/24000*50000 && MX(mm1)<410000-(MY(mm1)-18000)/24000*155000) 
            MI(mm1)=3;
        end
        % Asthenosphere below the oceanic plate
        if(MY(mm1)>60000 && MY(mm1)<=90000 && MX(mm1)>560000-(MY(mm1)-18000)/24000*75000 && MX(mm1)<440000+(MY(mm1)-18000)/24000*85000) 
            MI(mm1)=6;
        end
        ...
        yast=82000;                     % Bottom of the lithosphere
        ...
        if(MX(mm1)>400000 && MX(mm1)<600000 && MY(mm1)>10000 && MY(mm1)<yast)
            % T difference at the bottom of the oceanic and continental plates
            dt=-(ttop-tast)*(1-erf((yast-10000)/2/(kappa*age)^0.5));
            MTK(mm1)=tast+dt+(ttop-tast-dt)*(1-erf((MY(mm1)-10000)/2/(kappa*age)^0.5));
        end
        % Linear continental geotherm  
        if((MX(mm1)<=350000 || MX(mm1)>=650000) && MY(mm1)>7000 && MY(mm1)<yast)
            MTK(mm1)=ttop+(tast-ttop)*(MY(mm1)-7000)/(yast-7000);
        end
        % Transient left continent->ocean geotherm
        if(MX(mm1)>350000 && MX(mm1)<400000 && MY(mm1)>7000+(MX(mm1)-350000)/50000*3000 && MY(mm1)<yast)
            % Continental geotherm
            tcont=ttop+(tast-ttop)*(MY(mm1)-(7000+(MX(mm1)-350000)/50000*3000))/(yast-7000); % continental geotherm
            % T difference at the bottom of the oceanic and continental plates
            dt=-(ttop-tast)*(1-erf((yast-10000)/2/(kappa*age)^0.5));
            % Oceanic geotherm
            tocea=tast+dt+(ttop-tast-dt)*(1-erf((MY(mm1)-(7000+(MX(mm1)-350000)/50000*3000))/2/(kappa*age)^0.5)); % oceanic geotherm
            mwt=(MX(mm1)-350000)/50000; % Linear lateral transition 
...
for i=1:1:tnum
    ...
    gridt(2,i)=7000;
    % Above oceanic crust
    if(gridt(1,i)>=350000 && gridt(1,i)<=650000)
        gridt(2,i)=10000;
    end
    % Above left continent->ocean transition
    if(gridt(1,i)>350000 && gridt(1,i)<400000)
        gridt(2,i)=7000+(gridt(1,i)-350000)/50000*3000;
    end
    % Above ocean->right continent transition
    if(gridt(1,i)>600000 && gridt(1,i)<650000)
        gridt(2,i)=7000+(650000-gridt(1,i))/50000*3000;
    end
end
...
% Main Time cycle
ntimestep=0;
for ntimestep=ntimestep+1:1:stepmax
    ...
    for i=2:1:tnum-1
        ...
        if (gridt(1,i)>=gridx(1) && gridt(1,i)<=gridx(xnum));
            ...
            RA(i,1)=gridt(2,i)+gridt(5,i).*timestep;
        % External points
        else
    ...
    % Transition from Collision to breakof stage
    % Collision stage
    if(velcol>0)
        % Defining new gridline positions for irregular basic grid
        % Computing model size change
        xsize=xsize0-velcol*timesum; % Horizontal size
        ...
        % Adiabatic T increase in responce to model thickening
        for j=1:1:xnum
            % Lower boundary
            % tk(ynum,j)=bbottomt(j,1)+tk(ynum-1,j)*bbottomt(j,2)
    ...
    % Transition to Breakoff stage
    if(timesum>timestop && velcol>0)
        % Stop convergence
        velcol=0;
    ...
    % Save topography
    topotime(ntimestep,1)=timesum;
    topohigh(ntimestep,:)=gridt(2,:);
    topowater(ntimestep,1)=waterlev;
    
    % Save mat file
    if(fix(ntimestep/savestep)*savestep==ntimestep)
        namemat    =  ['collision',num2str(ntimestep)];
        save(namemat);
    end    
