...
% Setup corresponds to retreating subduction 
% 
% Staggered Grid 
% 
%     vx       vx       vx    
...
gridx(76)=200000; % initial position of the area
for i=77:1:176
    gridx(i)=gridx(i-1)+b;
end
...
N=75; % number of grid steps to be used in the grid
...
% of high resolution area
D=gridx(76); % distance to be covered by non-uniform grid
N=75; % number of grid steps to be used in the grid
% Iterative search of F
F=1.1;
for i=1:1:100
...
mxnum=1500; %total number of markers in horizontal direction
mynum=500;  %total number of markers in vertical direction
...
for xm = 1:1:mxnum
    for ym = 1:1:mynum
        ...
        % 5 = Lithosphere
        if((MX(mm1)<300000 && MY(mm1)>18000 && MY(mm1)<=40000) || (MX(mm1)>=300000 && MY(mm1)>18000 && MY(mm1)<=80000))
            MI(mm1)=5;
        end
        % 3 = Weak oceanic crust of the fracture zone (basalt)
        if(MX(mm1)>250000 && MX(mm1)<(300000+(18000-MY(mm1))*25) && MY(mm1)>14000 && MY(mm1)<=18000)
            MI(mm1)=3;
        end
        % 7 = Weak mantle of the fracture zone
        if(MX(mm1)>250000 && MX(mm1)<300000 && MY(mm1)>18000 && MY(mm1)<=60000)
            MI(mm1)=7;
        end
        ...
        if(MX(mm1)<300000 && MY(mm1)>10000 && MY(mm1)<100000)
            kappa=1e-6;                 % Thermal diffusivity of the mantle, m^2/s
            age=5e+6*(365.25*24*3600);  % Plate age, s
            t100=tbottom-dtdy*(ysize-100000); % T of asthenosphere at y=100 km
            MTK(mm1)=t100+(ttop-t100)*(1-erf((MY(mm1)-10000)/2/(kappa*age)^0.5));
         end
        % Oceanic geotherm in the right (older = 40 Myr) plate 
        % after Turcotte & Schubert (2002)
        if(MX(mm1)>300000 && MY(mm1)>10000 && MY(mm1)<200000)
            kappa=1e-6;                 % Thermal diffusivity of the mantle, m^2/s
            age=7e+7*(365.25*24*3600);  % Plate age, s
            t200=tbottom-dtdy*(ysize-200000); % T of asthenosphere at y=200 km
            MTK(mm1)=t200+(ttop-t200)*(1-erf((MY(mm1)-10000)/2/(kappa*age)^0.5));
         end
    end
end
    ...
    if(markmove>0)
        ...
        for mm1 = 1:1:marknum
            ...
            if (MX(mm1)>=0 && MX(mm1)<=xsize && MY(mm1)>=0 && MY(mm1)<=ysize) 
                ...
                % Define new trench position from mantle displacement
                if(MI(mm1)>=6 && MX(mm1)>trenchpos && MX(mm1)<=gridx(130) && MY(mm1)<20000)
                    trenchpos=MX(mm1);
                end
...
%     print ('-dtiff', '-r300','-zbuffer ','fig3');
    ...
    if(fix(ntimestep/savestep)*savestep==ntimestep)
        namemat    =  ['subduction_weak',num2str(ntimestep)];
        save(namemat);
    end    
    ...
    b=2000; % grid spacing in the high resolution area
    gridx(76)=trenchpos-100000; % Current position of the area
    for i=77:1:176
        gridx(i)=gridx(i-1)+b;
    end
    % Define factor of grid spacing increase to the right
