...
RX0=zeros(ynum(1)+1,xnum(1),znum(1)+1);
RY0=zeros(ynum(1),xnum(1)+1,znum(1)+1);
RZ0=zeros(ynum(1)+1,xnum(1)+1,znum(1));
RC0=zeros(ynum(1)-1,xnum(1)-1,znum(1)-1);
% Grid points cycle
for i=1:1:ynum(1);
    for j=1:1:xnum(1);
        for k=1:1:znum(1);
            ...
            if(i<ynum(1) && j>1 && k>1)
                if(i==1)
                    pr0(i,j-1,k-1)=prfirst;
                else
                    pr0(i,j-1,k-1)=pr0(i-1,j-1,k-1)+ystp(1)*g*(rho1(i,j,k)+rho1(i,j-1,k)+rho1(i,j,k-1)+rho1(i,j-1,k-1))/4;
                end
            end
            ...
            if(j>1 && k>1 && i>1 && i<ynum(1))
                RY0(i,j,k)=-g*(rho1(i,j,k)+rho1(i,j-1,k)+rho1(i,j,k-1)+rho1(i,j-1,k-1))/4;
            end
...
[vx0,resx0,vy0,resy0,vz0,resz0,pr0,resc0]=Stokes_Continuity3D_viscous_smoother(prfirst,etaxy1,etaxz1,etayz1,etan1,iternum(n),krelaxs,krelaxc,xnum(1),ynum(1),znum(1),xstp(1),ystp(1),zstp(1),RX0,vx0,RY0,vy0,RZ0,vz0,RC0,pr0);
RX1=resx0;
RY1=resy0;
RZ1=resz0;
RC1=resc0;
...
for niter=1:1:inum;
    ...
    for n=1:1:levelnum;
        ...
        switch n
            ...
            % by calling function Stokes_smoother()
            [vx1,resx1,vy1,resy1,vz1,resz1,pr1,resc1]=Stokes_Continuity3D_viscous_smoother(0,etaxy1,etaxz1,etayz1,etan1,iternum(n),krelaxs,krelaxc,xnum(n),ynum(n),znum(n),xstp(n),ystp(n),zstp(n),RX1,vx1,RY1,vy1,RZ1,vz1,RC1,pr1);
...
%     colorbar;
    ...
    if(etannn>etacyc)
        ...
        if (ynreset>1)
            ...
            etamaxcur=etamaxbeg;
            ynreset=0;
            %Computing, Saving current state of residuals
            % by calling function Stokes_smoother()
            vx0=vx0+vx1;
            vy0=vy0+vy1;
            vz0=vz0+vz1;
            pr0=pr0+pr1;
            [vx0,resx0,vy0,resy0,vz0,resz0,pr0,resc0]=Stokes_Continuity3D_viscous_smoother(prfirst,etaxyo1,etaxzo1,etayzo1,etano1,iternum(n),krelaxs,krelaxc,xnum(1),ynum(1),znum(1),xstp(1),ystp(1),zstp(1),RX0,vx0,RY0,vy0,RZ0,vz0,RC0,pr0);
            RX1=resx0;
            RY1=resy0;
            RZ1=resz0;
            RC1=resc0;
    ...
    figure(2);
    subplot(2,3,1)
    resx00=resx0/stokesscale;
    surf(resx00(:,:,(znum(1)-1)/2+1));
    shading interp;
    light;
    lighting phong;
    axis tight;
...
%     colorbar;
    ...
    % Plotting Residuals for y-Stokes as surface
    subplot(2,3,2)
    resy00=resy0/stokesscale;
    surf(resy00(:,:,(znum(1)-1)/2+1));
    shading interp;
    light;
    lighting phong;
    axis tight;
...
%     colorbar;
    ...
    % Plotting Residuals for z-Stokes as surface
    subplot(2,3,3)
    resz00=resz0/stokesscale;
    surf(resz00(:,:,(znum(1)-1)/2+1));
    %surf(resz2(:,:,(znum(1)-1)/4+1));
    shading interp;
    light;
    lighting phong;
...
%     colorbar;
    ...
    % Plotting Residuals for Continuity as surface
    subplot(2,3,4)
     resc00=resc0/continscale;
    surf(resc00(:,:,(znum(1)-1)/2+1));
    shading interp;
    light;
    lighting phong;
    axis tight;
    ...
    for i=1:1:ynum(1)
        for j=1:1:xnum(1)
            for k=1:1:znum(1)
                % x-Stokes
                if (i>1 && j>1 && k>1 && j<xnum(1))
                    resx000(niterbig)=resx000(niterbig)+resx00(i,j,k)^2;
                end
                % y-Stokes
                if (i>1 && j>1 && k>1 && i<ynum(1))
                    resy000(niterbig)=resy000(niterbig)+resy00(i,j,k)^2;
                end
                % z-Stokes
                if (i>1 && j>1 && k>1 && k<znum(1))
                    resz000(niterbig)=resz000(niterbig)+resz00(i,j,k)^2;
                end
                % Continuity
                if (j<xnum(1) && i<ynum(1) && k<znum(1))
                    resc000(niterbig)=resc000(niterbig)+resc00(i,j,k)^2;
                end
            end
        end
    end
    resx000(niterbig)=log10((resx000(niterbig)/((ynum(1)-1)*(xnum(1)-2)*(znum(1)-1)))^0.5);
    resy000(niterbig)=log10((resy000(niterbig)/((ynum(1)-2)*(xnum(1)-1)*(znum(1)-1)))^0.5);
    resz000(niterbig)=log10((resz000(niterbig)/((ynum(1)-1)*(xnum(1)-1)*(znum(1)-2)))^0.5);
    resc000(niterbig)=log10((resc000(niterbig)/((ynum(1)-1)*(xnum(1)-1)*(znum(1)-1)))^0.5);
