% Visco-elasto-plastic hydro-thermomechanical (HTM) code
% Solving momentum, mass and energy conservation eqs.
% for coupled fluid-solid system
...
rhosolidm   = [3300   3200   1     ]; % Solid Density, kg/m^3
etasolidm   = [1e+23  1e+17  1e+17 ]; % Solid Viscosity, Pa s
rhocpsolidm = [3.3e+6 3.2e+6 3.3e+6]; % Solid Volumetric heat capacity, kg/m^3
alphasolidm = [3e-5   2e-5   0     ]; % Solid Thermal expansion, 1/K
ksolidm     = [3      2      3000  ]; % Solid Thermal conductivity, W/m/K
hrsolidm    = [2e-8   3e-8   0     ]; % Solid Radiogenic heat production, W/m^3
gggsolidm   = [1e+11  1e+11  1e+11 ]; % Solid Shear Modulus, Pa
frictsolidm = [0.6    0      0     ]; % Solid Friction coefficient
cohessolidm = [1e+8   1e+8   1e+8  ]; % Solid compressive strength, Pa
tenssolidm  = [6e+7   6e+7   6e+7  ]; % Solid tensile strength, Pa
kphim0      = [1e-14  1e-14  1e-14 ]; % Standard permeability, m^2

% Fluid properties
etafluid=1e-3; % Fluid/Melt viscosity
rhofluid=1000; % Fluid/Melt density
rhocpfluid=3e+6; % Fluid/Melt volumetric isobaric heat capacity, J/K/m^3
alphafluid=5e-5; % Thermal expansion of the fluid
hrfluid=0e-7; % Fluid/Melt heat production, W/m^3
kfluid=5; % Fluid/Melt conductivity, W/m/K
...
% Timestepping
dtelastic=5e+7; % Maximal computational timestep, s
...
dxymax=0.01; % Max marker movement per time step, grid steps
...
yerrmax=3e+5; % Tolerance level for yielding error
YERRNOD=zeros(1,nplast); % Yielding error of nodes
etawt=0; % Weight for old viscosity
dphimax=0.1; % max porosity ratio change per time step
...
for m=1:1:marknum
    ...
    if(tm(m)<3)
        ...
        rhototalm=rhosolidm(tm(m))*(1-phim(m))+rhofluid*phim(m);
...
for j=1:1:Nx1
    for i=1:1:Ny1
        ...
        % Right part
        R(kqx)=min(rhofluid,RHOX(i,j))*gx;
        end
        ...
        % Right part
        R(kqy)=min(rhofluid,RHOY(i,j))*gy;
        end
...
for j=2:1:Nx
    for i=2:1:Ny
        APHI(i,j)=((pr(i,j)-pf(i,j))/ETAPHI(i,j)+...
         ((pr(i,j)-pr0(i,j))-(pf(i,j)-pf0(i,j)))/dtm*BETTAPHI(i,j))/(1-PHI(i,j))/PHI(i,j);
    end
end
...
for ii=2:1:Ny
    ...
    for rk=1:1:4
        ...
        wtmi1j1=(dxmj/dx)*(dymi/dy);
        % Compute vx, vy velocity
        vxn(rk)=vxpf(i,j)*wtmij+vxpf(i+1,j)*wtmi1j+...
            vxpf(i,j+1)*wtmij1+vxpf(i+1,j+1)*wtmi1j1;
        vyn(rk)=vypf(i,j)*wtmij+vypf(i+1,j)*wtmi1j+...
            vypf(i,j+1)*wtmij1+vypf(i+1,j+1)*wtmi1j1;
        ...
        wtmi1j1=(dxmj/dx)*(dymi/dy);
        % Compute vx velocity
        vxn(rk)=vpratio*vxn(rk)+(1-vpratio)*(vxf(i,j)*wtmij+vxf(i+1,j)*wtmi1j+...
            vxf(i,j+1)*wtmij1+vxf(i+1,j+1)*wtmi1j1);
        ...
        wtmi1j1=(dxmj/dx)*(dymi/dy);
        % Compute vx velocity
        vyn(rk)=vpratio*vyn(rk)+(1-vpratio)*(vyf(i,j)*wtmij+vyf(i+1,j)*wtmi1j+...
            vyf(i,j+1)*wtmij1+vyf(i+1,j+1)*wtmi1j1);        
...
subplot(2,1,1)
pcolor(xp/1000,yp/1000,pr-pf);% caxis([17 21])
shading flat;
axis ij image;
colorbar
title(['Ptotal-Pfluid, Pa timestep=',num2str(timestep),' time=',num2str(timesum/(365.25*24*3600)),' Yr'])
subplot(2,2,3)
pcolor(xp/1000,yp/1000,pr);% caxis([17 21])
shading flat;
axis ij image;
colorbar
title('Ptotal, Pa')
subplot(2,2,4)
pcolor(xp/1000,yp/1000,pf);% caxis([17 21])
shading flat;
axis ij image;
colorbar
title('Pfluid, Pa')
% % Save figure
% namefig=['i2elvis_3e5_' num2str(timestep)];
