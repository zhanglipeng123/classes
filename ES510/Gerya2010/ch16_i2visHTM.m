...
rhosolidm   = [3300   3250   3350    1     ]; % Solid Density, kg/m^3
etasolidm   = [1e+19  1e+19  1e+23   1e+17 ]; % Viscosity, Pa s
rhocpsolidm = [3.3e+6 3.2e+6 3.3e+6  3.3e+6]; % Solid volumetric heat capacity, kg/m^3
alphasolidm = [3e-5   2e-5   3e-5    0     ]; % Solid Thermal expansion, 1/K
...
rhofluidm=2700; % Fluid/Melt density
...
for m=1:1:marknum
    ...
    if(tm(m)<4)
        ...
        rhototalm=rhosolidm(tm(m))*(1-phim(m))+rhofluidm*phim(m);
        rhofluidm0=rhofluidm;
...
for j=2:1:Nx
    for i=2:1:Ny
        APHI(i,j)=(pr(i,j)-pf(i,j))/(1-PHI(i,j))/PHI(i,j)/ETAPHI(i,j);
