...
velext=2/(100*365.25*24*3600); % 1 cm/yr
...
for xm = 1:1:mxnum
    for ym = 1:1:mynum
        ...
        % 5 = Lithosphere (layering is to see deformation)
        if(MY(mm1)>23000 && MY(mm1)<=28000) 
            MI(mm1)=5;
        end
        if(MY(mm1)>33000 && MY(mm1)<=38000) 
            MI(mm1)=5;
        end
        if(MY(mm1)>43000 && MY(mm1)<=48000) 
            MI(mm1)=5;
        end
        if(MY(mm1)>53000 && MY(mm1)<=58000) 
            MI(mm1)=5;
        end
        if(MY(mm1)>63000 && MY(mm1)<=68000) 
            MI(mm1)=5;
        end
        if(MY(mm1)>73000 && MY(mm1)<=78000) 
            MI(mm1)=5;
        end
        if(MY(mm1)>83000 && MY(mm1)<=88000) 
            MI(mm1)=5;
        end
        ...
        % Thermal perturbation (10 K) in the base of the lithosphere
        % (to start extension in the middle of the model)
        dx=MX(mm1)-xsize/2;
        dy=MY(mm1)-80000;
        dx=(dx^2+dy^2)^0.5;
        if(dx<20000)
            MTK(mm1)=MTK(mm1)+10*(1-dx/20000);
        end
    end
end
...
%     print ('-dtiff', '-r300','-zbuffer ','fig3');
    ...
    ysize=ysize-velext/xsize*ysize*timestep; % Vertical size
    xsize=xsize+velext*timestep; % Horizontal size
    ...
    if(fix(ntimestep/savestep)*savestep==ntimestep)
        namemat    =  ['extension',num2str(ntimestep)];
        save(namemat);
    end    
