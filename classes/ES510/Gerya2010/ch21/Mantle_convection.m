...
% Setup corresponds to intrusion emplacement 
% 
% Staggered Grid 
% 
%     vx       vx       vx    
...
timemax=1e+5*365.25*24*3600; % 10 Myr
...
MFLOW(1,2)=1e+25;           % viscosity, Pa s
...
mxnum=200; %number of markers in horizontal direction
mxstep=xsize/mxnum; %step between markers in horizontal direction
mynum=200;  %number of markers in vertical direction   
...
for xm = 1:1:mxnum
    for ym = 1:1:mynum
        ...
        % 5,6 = Layered mantle
        MI(mm1)=5;
        xn=double(int16((3000000-MY(mm1))/(3000000)*20-0.5))+1;
        ...
        tast=1600; % Asthenospheric temperature extrapolated to the top of the model
        % Adiabatic temperature gradient in the asthenosphere 
        dtdy=0.5/1000; % 0.5 K/km
        
        % Adiabatic gradient with random perturbations
        MTK(mm1)=tast+dtdy*MY(mm1);
        % Oceanic geotherm (age = 70 Myr) plate 
        % after Turcotte & Schubert (2002)
        if(MY(mm1)<300000)
            kappa=1e-6;                 % Thermal diffusivity of the mantle, m^2/s
            age=7e+7*(365.25*24*3600);  % Plate age, s
            t200=tast+dtdy*MY(mm1); % T of asthenosphere at current depth
            MTK(mm1)=t200+(ttop-t200)*(1-erf((MY(mm1)-0)/2/(kappa*age)^0.5));
        end
        
        % Boundary layer at CMB
        if(MY(mm1)>2800000)
            MTK(mm1)=(tast+2800000*dtdy)+(tbottom-(tast+2800000*dtdy))*(MY(mm1)-2800000)/200000;
        end
...
fdata=fopen('convection.txt','rt'); 
xnumfile=51;
ynumfile=51;
tkfile=zeros(ynumfile,xnumfile);
xstpfile=xsize/(xnumfile-1);
ystpfile=ysize/(ynumfile-1);
% Horizontal grid in file
gridxfile=zeros(xnumfile,1);
for i=2:1:xnumfile
    gridxfile(i)=gridxfile(i-1)+xstpfile;
end
% Vertical grid in file
gridyfile=zeros(ynum,1);
for i=2:1:ynumfile
    gridyfile(i)=gridyfile(i-1)+ystpfile;
end
for i=1:1:ynumfile
    for j=1:1:xnumfile
...
fclose(fdata);
% Interpolating initial nodal temperatures to markers
% Marker cycle
for mm1 = 1:1:marknum
    % Check markers inside the grid
    if (MX(mm1)>=gridxfile(1) && MX(mm1)<=gridxfile(xnumfile) && MY(mm1)>=gridyfile(1) && MY(mm1)<=gridyfile(ynumfile)) 
        ...
        xn=double(int16(MX(mm1)./xstpfile-0.5))+1;
        yn=double(int16(MY(mm1)./ystpfile-0.5))+1;
        if (xn<1)
            xn=1;
        end
        if (xn>xnumfile-1)
        ...
        dx=(MX(mm1)-gridxfile(xn))./xstpfile;
        dy=(MY(mm1)-gridyfile(yn))./ystpfile;

        % Interpolate nodal temperature for the marker
        tkm=0;
        tkm=tkm+(1.0-dx).*(1.0-dy).*tkfile(yn,xn);
        tkm=tkm+(1.0-dx).*dy.*tkfile(yn+1,xn);
        tkm=tkm+dx.*(1.0-dy).*tkfile(yn,xn+1);
        tkm=tkm+dx.*dy.*tkfile(yn+1,xn+1);
...
% Previous convection time, s
timesum=868*(1e+6*365.25*24*3600); % 868 Myr
timestepd=timemax
...
% 
    ...
    subplot(4,1,2)
%     pcolor(gridx/1000,gridy/1000,tk1-273);
    pcolor(gridx/1000,gridy/1000,bb);
    shading interp;
    axis tight;
    title('dT')
    colorbar; 
    ...
    % Plotting dRHO
    subplot(4,1,4)
    pcolor(gridx/1000,gridy/1000,cc);
    shading interp;
    axis tight;
    title('dRHO')
    colorbar; 
...
%     end
        ...
        for m10 = m10min:1:m10max
            for m20 = m20min:1:m20max 
                ...
                if(dd<markdis(m20,m10))
                    ...
                    markgii(m20,m10)=max(MGII(mm1),1e-5);
    ...
    % Draw strain
    subplot(1,3,1)
    pcolor(mx,my,-log10(abs(markgii)));
    box on;
    colorbar;
    shading interp;
    axis ij image;
...
%     topowater(ntimestep,1)=waterlev;
    ...
    if(fix(ntimestep/savestep)*savestep==ntimestep)
        namemat    =  ['convection',num2str(ntimestep)];
        save(namemat);
    end    
