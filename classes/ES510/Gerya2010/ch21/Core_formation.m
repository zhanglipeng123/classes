...
timemax=1e+4*365.25*24*3600; % 10 Myr
...
% 
% % Creating matrix for Poisson equation
% LP=sparse(xnum*ynum,xnum*ynum);
% RP=zeros(xnum*ynum,1);
...
% 
    ...
    [LP,RP,fi1,resp1]=Poisson_solver_planet_grid(LP,RP,xnum-1,ynum-1,gridcx(2:1:xnum),gridcy(2:1:ynum),RP1,xcentre,ycentre,rbon,vbon);
...
%     topowater(ntimestep,1)=waterlev;
     % Save mat file
    if(ntimestep==1 || fix(ntimestep/savestep)*savestep==ntimestep)
        namemat    =  ['core_',num2str(ntimestep)];
        save(namemat);
    end    
