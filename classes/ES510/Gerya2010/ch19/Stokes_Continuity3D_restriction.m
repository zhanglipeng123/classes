% Function Stokes_Continuity3D_restriction()
...
function[RX,RY,RZ,RC]=Stokes_Continuity3D_restriction(n,xnum,ynum,znum,xstp,ystp,zstp,resx,resy,resz,resc)
% 
    ...
    for j=2:1:xnum(n);
        for k=2:1:znum(n);
            ...
            if (i<ynum(n) && j<xnum(n) && k<znum(n));
                ...
                % Add residual from the finer level to right parts of the coarcer level 
                % for 8 nodes bounding the cell
                RC(ic,jc,kc)=RC(ic,jc,kc)+(1.0-dx)*(1.0-dy)*(1.0-dz)*resc(i,j,k);
                wtc(ic,jc,kc)=wtc(ic,jc,kc)+(1.0-dx)*(1.0-dy)*(1.0-dz);
                RC(ic,jc,kc+1)=RC(ic,jc,kc+1)+(1.0-dx)*(1.0-dy)*dz*resc(i,j,k);
                wtc(ic,jc,kc+1)=wtc(ic,jc,kc+1)+(1.0-dx)*(1.0-dy)*dz;
                RC(ic+1,jc,kc)=RC(ic+1,jc,kc)+(1.0-dx)*dy*(1.0-dz)*resc(i,j,k);
                wtc(ic+1,jc,kc)=wtc(ic+1,jc,kc)+(1.0-dx)*dy*(1.0-dz);
                RC(ic+1,jc,kc+1)=RC(ic+1,jc,kc+1)+(1.0-dx)*dy*dz*resc(i,j,k);
                wtc(ic+1,jc,kc+1)=wtc(ic+1,jc,kc+1)+(1.0-dx)*dy*dz;
                RC(ic,jc+1,kc)=RC(ic,jc+1,kc)+dx*(1.0-dy)*(1.0-dz)*resc(i,j,k);
                wtc(ic,jc+1,kc)=wtc(ic,jc+1,kc)+dx*(1.0-dy)*(1.0-dz);
                RC(ic,jc+1,kc+1)=RC(ic,jc+1,kc+1)+dx*(1.0-dy)*dz*resc(i,j,k);
                wtc(ic,jc+1,kc+1)=wtc(ic,jc+1,kc+1)+dx*(1.0-dy)*dz;
                RC(ic+1,jc+1,kc)=RC(ic+1,jc+1,kc)+dx*dy*(1.0-dz)*resc(i,j,k);
                wtc(ic+1,jc+1,kc)=wtc(ic+1,jc+1,kc)+dx*dy*(1.0-dz);
                RC(ic+1,jc+1,kc+1)=RC(ic+1,jc+1,kc+1)+dx*dy*dz*resc(i,j,k);
    ...
    for jc=1:1:xnum(n+1)+1;
        for kc=1:1:znum(n+1)+1;
            ...
            if (ic<ynum(n+1) && jc<xnum(n+1) && kc<znum(n+1));
                if (wtc(ic,jc,kc)~=0);
                    RC(ic,jc,kc)=RC(ic,jc,kc)/wtc(ic,jc,kc);
                else
                    RC(ic,jc,kc)=0;
                end
            end
