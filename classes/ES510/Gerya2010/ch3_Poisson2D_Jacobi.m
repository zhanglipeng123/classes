...
PHI=zeros(ynum,xnum); % current
PHInew=zeros(ynum,xnum); % new
...
for k=1:1:niter
    ...
    for i=2:1:ynum-1
      for j=2:1:xnum-1
            ...
            % Compute new value for current unknown
            PHInew(i,j)=PHI(i,j)+dR(i,j)/(-2/xstp^2-2/ystp^2)*TETA;
      end
    end

    % Assign new values to all unknowns
    PHI=PHInew;
