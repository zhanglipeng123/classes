...
EPS=zeros(Ny,Nx); % Plastic strain
% Vx-Nodes
...
pr0m=zeros(1,marknum); % Pressure, Pa
vx0m=zeros(1,marknum); % vx, m/s
vy0m=zeros(1,marknum); % vy, m/s
epsplm=zeros(1,marknum); % Plastic strain

% Define properties of materials: 
%        Bridge  Ball   Air
rhom    = [3000   3000   1     ]; % Density, kg/m^3
etam    = [1e+22  1e+20  1e-3  ]; % Viscosity, Pa s
rhocpm  = [3.0e+6 3.0e+6 3.0e+6]; % Volumetric heat capacity, kg/m^3
alpham  = [3e-5   3e-5   0     ]; % Thermal expansion, 1/K
bettam  = [1e-9   1e-9   1e-9  ]; % Compressibility, 1/Pa
km      = [3      2      3000  ]; % Thermal conductivity, W/m/K
hrm     = [2e-8   3e-8   0     ]; % Radiogenic heat production, W/m^3
gggm    = [1e+9   1e+9   1e+9  ]; % Shear Modulus, Pa
frict0m = [0.6    0.6    0.6   ]; % Initial friction coefficient
frict1m = [0.6    0.6    0.6   ]; % Final friction coefficient
cohes0m = [6e+7   2e+8   3e+5  ]; % Initial compressive strength, Pa
cohes1m = [6e+6   2e+8   3e+5  ]; % Final compressive strength,, Pa
tens0m  = [3e+7   1e+8   3e+5  ]; % Initial compressive strength, Pa
tens1m  = [3e+5   1e+8   3e+5  ]; % Final compressive strength,, Pa
epspl0m = [0      0      0     ]; % Min plastic strain limit for weakening
epspl1m = [0.1    1      1     ]; % Max plastic strain limit for weakening
...
for jm=1:1:Nxm
    for im=1:1:Nym
        ...
        % Bridge
        if((xm(m)>0.10*xsize && ym(m)>ysize-xsize/4-ysize*0.15 && ym(m)<ysize-xsize/4) ||...
            (xm(m)>0.80*xsize && ym(m)>ysize-xsize/4-ysize*0.1))
            tm(m)=1; % Material type
        end
        % Ball
        xcirc=xsize/4+xsize*0.10;
        ycirc=ysize/4+ysize*0.07;
        rcirc=xsize/4;
        drm=((xm(m)-xcirc)^2+(ym(m)-ycirc)^2)^0.5;
        if(drm<rcirc)
            tm(m)=2; % Material type
            vy0m(m)=20; % Vertical velocity, m/s
        end
        % Initial temperature
        tkm(m)=273;
...
dtmin=1e-9; % Minimal computational timestep, s
dt=0.001; % Current computational timestep, s
...
dtstep=50; % Number of iterations before changing dt
...
DPmax=3e+7; % Max stress change per time step, Pa
...
yerrmax=3e+4; % Tolerance level for yielding error, Pa
terrmax=1e-3; % Tolerance level for temperature error, K
perrmax=1e+6; % Tolerance level for pressure error, Pa
...
for m=1:1:marknum
    % Compute marker cohesion and friction
    if(epsplm(m)<=epspl0m(tm(m)))
        % Initial strength
        cohesm=cohes0m(tm(m));
        frictm=frict0m(tm(m));
        tensm=tens0m(tm(m));
    elseif(epsplm(m)<epspl1m(tm(m)))
        % Strain weakening
        kfpl=(epsplm(m)-epspl0m(tm(m)))/(epspl1m(tm(m))-epspl0m(tm(m)));
        cohesm=cohes0m(tm(m))+(cohes1m(tm(m))-cohes0m(tm(m)))*kfpl;
        frictm=frict0m(tm(m))+(frict1m(tm(m))-frict0m(tm(m)))*kfpl;
        tensm=tens0m(tm(m))+(tens1m(tm(m))-tens0m(tm(m)))*kfpl;
    else
        % Final strength
        cohesm=cohes1m(tm(m));
        frictm=frict1m(tm(m));
    ...
    COHSUM(i,j)=COHSUM(i,j)+cohesm*wtmij;
    TENSUM(i,j)=TENSUM(i,j)+tensm*wtmij;
    FRISUM(i,j)=FRISUM(i,j)+frictm*wtmij;
    EPSSUM(i,j)=EPSSUM(i,j)+epsplm(m)*wtmij;
    ...
    COHSUM(i+1,j)=COHSUM(i+1,j)+cohesm*wtmi1j;
    TENSUM(i+1,j)=TENSUM(i+1,j)+tensm*wtmi1j;
    FRISUM(i+1,j)=FRISUM(i+1,j)+frictm*wtmi1j;
    EPSSUM(i+1,j)=EPSSUM(i+1,j)+epsplm(m)*wtmi1j;
    ...
    COHSUM(i,j+1)=COHSUM(i,j+1)+cohesm*wtmij1;
    TENSUM(i,j+1)=TENSUM(i,j+1)+tensm*wtmij1;
    FRISUM(i,j+1)=FRISUM(i,j+1)+frictm*wtmij1;
    EPSSUM(i,j+1)=EPSSUM(i,j+1)+epsplm(m)*wtmij1;
    ...
    COHSUM(i+1,j+1)=COHSUM(i+1,j+1)+cohesm*wtmi1j1;
    TENSUM(i+1,j+1)=TENSUM(i+1,j+1)+tensm*wtmi1j1;
    FRISUM(i+1,j+1)=FRISUM(i+1,j+1)+frictm*wtmi1j1;
    EPSSUM(i+1,j+1)=EPSSUM(i+1,j+1)+epsplm(m)*wtmi1j1;
...
ETA5=ETA0;
YNY5=zeros(Ny,Nx);
SIIB=zeros(Ny,Nx); % Second stress invariant, Pa
DSY=zeros(Ny,Nx);
ynpl=0;
ddd=0;
for i=1:1:Ny
  for j=1:1:Nx
    ...
    SIIB(i,j)=(SXY(i,j)^2+((SXX(i,j)+SXX(i+1,j)+SXX(i,j+1)+SXX(i+1,j+1))/4)^2)^0.5;
    % Compute second invariant for a purely elastic stress build-up
    siiel=SIIB(i,j)*(GGG(i,j)*dt+ETA(i,j))/ETA(i,j);
    % Compute pressure
    prB=(pr(i,j)+pr(i+1,j)+pr(i,j+1)+pr(i+1,j+1))/4;
    % Compute yielding stress
    syieldc=COH(i,j)+FRI(i,j)*prB; % Confined fracture
    syieldt=TEN(i,j)+prB; % Tensile fracture
    syield=max(min(syieldc,syieldt),0); % Non-negative strength requirement
...
% Visualise results
if(fix(iplast/visstep)*visstep==iplast || (YERRNOD(iplast)<yerrmax && ynpl>0))

figure(3);colormap('Jet');clf
subplot(2,4,1)
pcolor(x,y,log10(ETA0));% caxis([17 21])
shading flat;
axis ij image;
colorbar
...
subplot(2,4,2)
pcolor(x,y,log10(ETA00));% caxis([17 21])
shading flat;
axis ij image;
colorbar
title(['log10ETA00, Pa*s timestep=',num2str(timestep)])
...
title(['log Yielding error=',num2str(YERRNOD(iplast)),' Pa for ',num2str(ynpl),' nodes'])
...
% Interpolate new viscoplastic viscosity and plastic strain rate to markers
for m=1:1:marknum
...
%                 1/ETA(i,j+1)*wtmij1+1/ETA(i+1,j+1)*wtmi1j1);
        ...
        if(etavpm(m)>=etam(tm(m)))
            ...
            % Interpolate plastic strain
            epsplm(m)=epsplm(m)+dt*0.5*max(0,...
              YNY(i,j)*SIIB(i,j)*(1/ETA(i,j)-1/ETA0(i,j))*wtmij+...
              YNY(i+1,j)*SIIB(i+1,j)*(1/ETA(i+1,j)-1/ETA0(i+1,j))*wtmi1j+...
              YNY(i,j+1)*SIIB(i,j+1)*(1/ETA(i,j+1)-1/ETA0(i,j+1))*wtmij1+...
              YNY(i+1,j+1)*SIIB(i+1,j+1)*(1/ETA(i+1,j+1)-1/ETA0(i+1,j+1))*wtmi1j1);
        end
    else
        etavpm(m)=etam(tm(m));
    end
    epsplm(m)=max(0,epsplm(m));
end
...
for m=1:1:marknum
    ...
    dsxxm1=dsxxm0*exp(-dsubgrids*dtm/(etavpm(m)/gggm(tm(m))));
    dprm1=dprm0*exp(-dsubgrids*dtm/(etavpm(m)/gggm(tm(m))));
...
for m=1:1:marknum
    ...
    % Interpolate vx at 1st timestep
    if(timestep==1)
    ...
    % Interpolate vy at 1st timestep
    if(timestep==1)
...
pcolor(x,y,log10(EPS));% caxis([17 21])
shading flat;
axis ij image;
colorbar
title(['log10Strain timestep=',num2str(timestep),' time=',num2str(timesum),' s'])
...
% Save figure
namefig=['i2elvisNS_' num2str(timestep)];
