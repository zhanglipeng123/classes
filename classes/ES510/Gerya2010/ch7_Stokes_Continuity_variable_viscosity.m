...
% Define viscosity and density in basic nodes
for i=1:1:Ny
  for j=1:1:Nx
    ...
    if(x(j)<xsize/2)
        ...
        ETA(i,j)=1e+20;  % viscosity in basic nodes 
    else
        % Right layer
        RHO(i,j)=3300;  % density
        ETA(i,j)=1e+22;  % viscosity in basic nodes 
    end
  end
end

% Introducing scaled pressure
pscale=1e+20/(dx+dy)*2;
...
for j=1:1:Nx1
    for i=1:1:Ny1
        ...
        L(kvy,kvx-Ny1*3)=ETA1/dx/dy; %Vx1
        L(kvy,kvx+3-Ny1*3)=-ETA1/dx/dy; %Vx2
