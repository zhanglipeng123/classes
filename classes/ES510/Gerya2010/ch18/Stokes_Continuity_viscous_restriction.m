% Function Stokes_Continuity_viscous_restriction()
% This function makes restriction operation: 
% Interpolates residuals (resx,resy,resc) from finer (k) to coarcer (k+1) level
% using bilinear interpolation and normal viscosity (etanf, etanc) for both levels
...
function[RX,RY,RC]=Stokes_Continuity_viscous_restriction(k,xnum,ynum,xstp,ystp,resx,resy,resc,etanf,etanc)
% 
...
for i=2:1:ynum(k);
    for j=2:1:xnum(k);
        ...
        if (i<ynum(k) && j<xnum(k));
            ...
            % Add residual from the finer level to right parts of the coarcer level 
            % for 4 nodes bounding the cell
            RC(ic,jc)=RC(ic,jc)+(1.0-dx)*(1.0-dy)*resc(i,j)*etanf(i,j);
            wtc(ic,jc)=wtc(ic,jc)+(1.0-dx)*(1.0-dy);
            RC(ic,jc+1)=RC(ic,jc+1)+dx*(1.0-dy)*resc(i,j)*etanf(i,j);
            wtc(ic,jc+1)=wtc(ic,jc+1)+dx*(1.0-dy);
            RC(ic+1,jc)=RC(ic+1,jc)+(1.0-dx)*dy*resc(i,j)*etanf(i,j);
            wtc(ic+1,jc)=wtc(ic+1,jc)+(1.0-dx)*dy;
            RC(ic+1,jc+1)=RC(ic+1,jc+1)+dx*dy*resc(i,j)*etanf(i,j);
...
for ic=1:1:ynum(k+1)+1;
    for jc=1:1:xnum(k+1)+1;
        ...
        if (ic<ynum(k+1) && jc<xnum(k+1));
            if (wtc(ic,jc)~=0);
                RC(ic,jc)=RC(ic,jc)/wtc(ic,jc)/etanc(ic,jc);
            else
                RC(ic,jc)=0;
            end
        end
