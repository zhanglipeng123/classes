...
% Setup corresponds to viscoelastic slab bending/unbending
% 
% Staggered Grid 
% 
%     vx       vx       vx    
...
MRHO(1,3)=ttop;   % T0
MRHO(2,1)=1;               % standard density, kg/m^3
MRHO(2,2)=0;             % thermal expansion, 1/K
MRHO(2,3)=ttop;   % T0
MRHO(3,1)=4000;               % standard density, kg/m^3
MRHO(3,2)=0;             % thermal expansion, 1/K
MRHO(3,3)=ttop;   % T0% Rock flow law: 
MRHO(4,1)=4000;               % standard density, kg/m^3
...
MFLOW(2,1)=0;
MFLOW(2,2)=1e+21;     % ETA, Pa s
MFLOW(3,1)=0;
MFLOW(3,2)=1e+27;     % ETA, Pa s
MFLOW(4,1)=0;
MFLOW(4,2)=1e+27;     % ETA, Pa s
...
timemax=2e+2*(365.25*24*3600);
% Number of timesteps before switching off gravity
gysteps=100;
% Number of timesteps before increasing the timestep
ttsteps=250;
...
for xm = 1:1:mxnum
    for ym = 1:1:mynum
        ...
        m1=double(int16(MX(ym,xm)/(xsize/20)-0.5))+1;
        m1=m1-double(int16(m1/2-0.5))*2;
        m2=double(int16(MY(ym,xm)/(ysize/20)-0.5))+1;
        m2=m2-double(int16(m2/2-0.5))*2;
        if(m2==m1)
            MI(ym,xm)=2;
        end
        % Slab
        if (MY(ym,xm)/ysize>0.2 && MY(ym,xm)/ysize<0.8 && MX(ym,xm)/xsize<0.8)
            MI(ym,xm)=MI(ym,xm)+2;
        end
...
%    pcolor(gridcx(2:1:xnum)/1000,gridcy(2:1:ynum)/1000,sxx2);
    ...
    hold on;
    % Plotting velocity
    quiver(gridx(2:2:xnum-1)/1000,gridy(2:2:ynum-1)/1000,vxb(2:2:ynum-1,2:2:xnum-1),vyb(2:2:ynum-1,2:2:xnum-1),'k');
    hold off;
    axis ij image;
...
%     print ('-dtiff', '-r300','-zbuffer ','fig3');
    ...
    % Draw composition
    if (ntimestep>gysteps)
        figure(4);
    else
        figure(3);
    end
    pcolor(mx,my,-(markcom./4).^2+1);
    colormap(Gray);
    shading interp;
    axis ij image;
    title(['Step=',num2str(ntimestep),' Myr=',num2str(timesum*1e-6/(365.25*24*3600))]); 
