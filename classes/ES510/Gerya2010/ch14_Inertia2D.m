...
% Define Gravity
gx=4; % Horizontal gravity acceleration, m/s^2
...
%        Block  Indentor Air
rhom   = [3000   7000   1     ]; % Density, kg/m^3
etam   = [1e+20  1e+22  1e-3  ]; % Viscosity, Pa s
rhocpm = [3.0e+6 3.0e+6 3.0e+6]; % Volumetric heat capacity, kg/m^3
alpham = [3e-5   3e-5   0     ]; % Thermal expansion, 1/K
bettam = [1e-8   1e-8   1e-8 ]; % Compressibility, 1/Pa
km     = [3      2      3000  ]; % Thermal conductivity, W/m/K
hrm    = [2e-8   3e-8   0     ]; % Radiogenic heat production, W/m^3
gggm   = [1e+8   1e+8   1e+8  ]; % Shear Modulus, Pa
frictm = [0.6    0.6    0.6     ]; % Friction coefficient
cohesm = [1e+8   1e+8   1e+8  ]; % Cohesion, Pa
...
%         % Block
%         if(ym(m)>ysize*0.8 && xm(m)>xsize*0.0 && xm(m)<xsize*1.0)
...
%         if((ym(m)>ysize*0.05 && ym(m)<ysize*0.25 &&...
%            xm(m)>xsize*0.35 && xm(m)<xsize*0.65) ||... 
%            (ym(m)>ysize*0.25 &&...
%            xm(m)>xsize*0.35+(ym(m)-ysize*0.25) && xm(m)<xsize*0.65-(ym(m)-ysize*0.25)))
%             tm(m)=2; % Material type
%         end
        % Initial temperature
        tkm(m)=273;
...
perrmax=3e+5; % Tolerance level for pressure error, Pa
...
for m=1:1:marknum
    ...
    FRISUM(i,j)=FRISUM(i,j)+frictm(tm(m))*wtmij;
    ...
    FRISUM(i+1,j)=FRISUM(i+1,j)+frictm(tm(m))*wtmi1j;
    ...
    FRISUM(i,j+1)=FRISUM(i,j+1)+frictm(tm(m))*wtmij1;
    ...
    FRISUM(i+1,j+1)=FRISUM(i+1,j+1)+frictm(tm(m))*wtmi1j1;
...
% Save figure
namefig=['Inertia1e9_' num2str(timestep)];
